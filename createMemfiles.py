...
def main():
    ...
    for i in memfiles[6:]:
        ...
        if tmp[1][0] == '<':
            if addr >= 0x90000000:
                outfile_boot.write('@%X \t%s' % (((addr&addr_mask)/4), newlinetag))
                #low 5 hex digits of the address
        ...
        if addr >= 0x90000000:
            ...
            outfile_programmif.write('%X : %08x; %s'\
                % (((addr&addr_mask)/4), instruction, newlinetag))
